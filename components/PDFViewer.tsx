        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    {/* PDF Modal */}
                    <motion.div
                        className={`fixed z-50 bg-[#1e1e1e] rounded-xl overflow-hidden shadow-2xl
                            border border-white/10 flex flex-col
                            ${isFullscreen
                                ? "inset-2 md:inset-4"
                                : "inset-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-[90vw] md:max-w-4xl md:h-[85vh]"
                            }`}
                    ...
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between h-12 px-4 bg-[#323233] border-b border-white/5 flex-shrink-0">
                            {/* Traffic lights */}
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => setIsFullscreen(!isFullscreen)}
                                    className="p-1.5 rounded-md hover:bg-white/10 transition-colors text-white/60 hover:text-white hidden md:block"
                                >
                                ...
                                <button
                                    onClick={onClose}
                                    className="p-1.5 rounded-md hover:bg-white/10 transition-colors text-white/60 hover:text-white md:hidden"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
